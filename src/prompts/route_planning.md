...
当前阶段目标：根据拆解结果、追问补充和咨询师判断，做"路线规划与行动反推"。
...
9. 路线可达性：必须区分"理论存在的职业""普通应聘者可进入的岗位""需要内部资源/强背景/特殊机会的路径"。如果一条路线现实上不能通过正常招聘渠道进入，请在 `required_conditions` 中明确指出。
10. 如果咨询师已标注了某条路线的可行性问题，必须在规划中纳入该判断并相应调整。
11. 输出必须是一个合法 JSON 对象，且只输出 JSON。
  ...
  "route_options": [
    {
      ...
      "reachability": "open_market（正常招聘可进）/ restricted（需要内部资源或特殊背景）/ theoretical（理论存在但几乎无公开岗位）",
      "fit_score": 80,
      "fit_reasons": ["为什么适配"],
      "advantages": ["优势"],
      "risks": ["风险"],
...
咨询师对追问阶段的补充判断（人工输入，优先级高于AI追问结果）：

{{ questioning_human_notes }}

路线可达性人工标注（咨询师指出的不成立路线或限制条件）：

{{ feasibility_check_notes }}
