...
class OpenAICompatibleClient(BaseLLMClient):
    ...
    Works against SiliconFlow, DeepSeek's direct API, OpenAI, or any gateway
    that speaks the OpenAI chat-completions protocol. The provider is selected
    purely through ``Settings`` (base URL, API key, model), so swapping vendors
    ...
    def client(self) -> OpenAI:
        if not self.is_configured():
            raise RuntimeError(
                "Missing LLM API key. Set LLM_API_KEY (or the legacy "
                "SILICONFLOW_API_KEY) before calling the model."
            )
    ...
    def generate_json(
        ...
        *,
        ...
        if self.settings.native_json_mode:
            try:
                completion = self._create_completion(
                    model=active_model,
                    temperature=temperature,
                ...
                # Endpoint rejected response_format; fall back to plain generation.
                raw_text = self._generate_json_plain(
                    system_prompt,
                    user_prompt,
                    model=active_model,
                    temperature=temperature,
...
# Backwards-compatible alias. Older imports referenced ``SiliconFlowClient``.
